...
class BarrierEnv4(gym.Env):
    ...
    def __init__(self,
        ...
        # State (observation) space for Gym [action, price, barrdist, ttm]
        self.observation_space = spaces.Box(low = np.array([0, -20, 0, 0]), high = np.array([1, 20, 1, 1]) )
    ...
    def _initiate_state(self):
        return [0, self.generator.current, self.generator.current-self.barrier, self.expiry]

    def normalize_state(self, state):
        normalized = [state[0]/(self.max_action-self.min_action) , 10*(math.log(state[1]/100)-self.generator.r), state[2]/(120-self.barrier) , state[3]/self.expiry]
        return normalized

    def denormalize_state(self, state):
        denorm = [state[0]*(self.max_action-self.min_action), 100*math.exp(state[1]/10.0+self.generator.r), state[2]*(120-self.barrier), state[3]*self.expiry]
        return denorm
    ...
    def step(self, action):
        ...
        # Calc new portfolio value
        new_und = self.generator.get_next()
        self.state = [action, new_und, new_und - self.barrier ,self.state[-1] - 1]
        new_barrier_value = self._get_derivative_value()
        new_static_hedge_value = self.n_puts_sold * self.generator.get_option_value(self.put_strike, self.state[-1], call=False)
