...
class BarrierEnv2(gym.Env):
    ...
    def _get_current_delta(self):
        return self.generator.get_DIP_delta(self.state[1], self.K, self.state[2]) - self.n_puts_sold*self.generator.get_delta(self.state[1], self.put_strike, self.state[2])

    def _initiate_state(self):
        return [0, self.generator.current, self.expiry, self._get_current_delta()]
    ...
    def step(self, action):
        ...
        # Calc new portfolio value
        new_und = self.generator.get_next()
        self.state = [action, new_und, self.state[2] - 1, self._get_current_delta()]
        new_barrier_value = self._get_derivative_value()
