...
import numpy as np
import pandas as pd
from Models import DeltaHedge, ThresholdDelta
...
if __name__ == "__main__":
    ...
    modelPPO = model.load('./networks/PPO/r2/30day/weights_PPO.zip')


    # generator2 = GBM_Generator(S0, r, sigma, freq, seed=124, barrier=barrier) # 
    # test_env_args2 = {
    ...
    test_envDQN = BarrierEnv2(**test_env_args)
    model2 = DQN(policy="MlpPolicy", env=test_envDQN)
    modelDQN = model2.load('./networks/DQN/r2/30day/weights_DQN.zip')
    ...
    dfDelta2["Agent"] = "Threshold Delta"

    df = pd.concat([dfPPO, dfDQN, dfDelta, dfDelta2], ignore_index=False) # dfDQN, 
    df['time'] = df.index
    df.reset_index(inplace=True)
    Utils.plot_decisions2(df)
    #Utils.plot_decisions_extra(dfDelta, dfPPO)
    n_sim=100
    ...
    outDelta2 = Utils.simulate_pnl("Threshold Delta", n_sim, test_envPPO, threshold_delta_agent.test)
    #outDQN = Utils.simulate_pnl("DQN", n_sim, test_envDQN, simulateDQN)
    #outDelta = Utils.simulate_pnl("Delta", n_sim, test_envPPO, delta_agent.test)
    out = pd.concat([outPPO, outDQN, outDelta, outDelta2], ignore_index=True) # , outDQN
