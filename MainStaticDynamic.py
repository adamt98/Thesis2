...
sigma = 0.012*np.sqrt(250) # 1% vol per day, annualized
r = 0.0 # Annualized
S0 = 100
...
if __name__ == "__main__":
    ...
    for spot in spots:
        ...
        barrier = 90
        generator = GBM_Generator(spot, r, 0.2, 1, barrier=barrier)
        val = generator.get_barrier_value(K=100, ttm=30, up=False, out=False, call=False)
        val2 = generator.get_option_value(K=100, ttm=30, call=False)
        delta_barr = generator.get_DIP_delta(spot, K=100, ttm=30) 
        
        delta_static = generator.get_delta(spot, 100, 30) - 1 # get_delta gives call delta, we're selling a put
    ...
    plt.plot(spots, delta2)
    plt.show()

    plt.plot(spots, vals)
    plt.plot(spots, vals2)
    plt.show()

    diffs = np.array(deltas) - np.array(delta2)
    plt.plot(spots, diffs)
    plt.show()
