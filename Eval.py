...
if __name__ == "__main__":
    ...
    test_envPPO = DiscreteEnv(**test_env_args)
    model = PPO(policy="MlpPolicy", env=test_envPPO)
    modelPPO = model.load('./networks/PPO/vanilla/c0/weights_PPO.zip')
    ...
    test_env_args2 = {
        ...
        "kappa" : kappa,
        "cost_multiplier" : cost_multiplier,
        "reward_type" : "basic", # "static" or "basic"
        "testing" : True,
        # "n_puts_sold" : n_puts_sold,
        # "min_action" : min_action,
        # "max_action" : max_action,
    ...
    test_envDQN = DiscreteEnv(**test_env_args2)
    model2 = DQN(policy="MlpPolicy", env=test_envDQN)
    modelDQN = model2.load('./networks/DQN/vanilla/c0/weights_DQN.zip')
    ...
    delta_agent = DeltaHedge(generator.initial, call = True)#, n_puts_sold=n_puts_sold, min_action=min_action, put_K=put_strike) # , n_puts_sold=n_puts_sold, min_action=min_action &&& call->False # for barr3  put_K = put_strike
    obs = test_envPPO.reset()
    dfDelta = delta_agent.test(test_envPPO, obs)
    dfDelta["Agent"] = "Delta"

    df = pd.concat([dfPPO, dfDQN, dfDelta], ignore_index=False) # dfDQN, 
    df['time'] = df.index
    df.reset_index(inplace=True)
    Utils.plot_decisions2(df)

    n_sim=100
    outPPO = Utils.simulate_pnl("PPO", n_sim, test_envPPO, simulatePPO)
    outDQN = Utils.simulate_pnl("DQN", n_sim, test_envDQN, simulateDQN)
    outDelta = Utils.simulate_pnl("Delta", n_sim, test_envDQN, delta_agent.test)
    #outDQN = Utils.simulate_pnl("DQN", n_sim, test_envDQN, simulateDQN)
    #outDelta = Utils.simulate_pnl("Delta", n_sim, test_envPPO, delta_agent.test)
    out = pd.concat([outPPO, outDQN, outDelta], ignore_index=True) # , outDQN
